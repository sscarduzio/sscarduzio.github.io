...
var jr = {
	...
	styles : [
		'themes/scarduzio.css',
		'themes/normalize.css',
		'themes/skeleton.css'
	],
	template: "themes/tpl2.html",
	scripts : [
		'js/showdown.js'
...
jr.wrap = function() {
	ajax(jr.template, function(tplHtml){
		var articleDiv = document.createElement("div");
		articleDiv.className += " content";
		while (document.body.firstChild) { articleDiv.appendChild(document.body.firstChild); }

		// Set page title 
		document.body.innerHTML  = tplHtml
		var firstH1 = articleDiv.getElementsByTagName('h1')[0]
		document.title = firstH1.innerHTML

		// Title links to this page
		firstH1.innerHTML = '<a href="' + window.location.href.toString() + '">' + firstH1.innerHTML + '</a>'

		// Place the html-ized markdown in the placeholder
		var articlePlace = document.getElementById("___content___")
		var articlePlaceParent = articlePlace.parentNode
		articlePlaceParent.removeChild(articlePlace)
		articlePlaceParent.appendChild(articleDiv)
	});
};
...
jr.loadBlock = function(file, selector, callback) {
